...
const PasswordReset = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setIsLoading(true);
      const response = await AuthService.resetPassword(password, passwordResetToken);
  ...
  return (
    <div className="relative flex justify-center items-center min-h-screen bg-darkblue text-white overflow-hidden">
      ...
      {isSuccess &&(
        <div className="absolute top-4 left-1/2 transform -translate-x-1/2 bg-blue-500 text-white px-4 py-2 rounded-md shadow-lg animate-fade-in-down">
        {isSuccess}
      </div>
      )}
